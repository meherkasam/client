 ...
 * @author meher
 * 
 *         This class executes all commands on behalf of the user. It contains
 *         functions to directly to the network stream as well as functions that
 *         perform file operations on the server through requests.
 * 
 */
...
public class CommandExecuter {
	...
	static DataObject Get(DataObject a, String fileName, int priority, int limit) {
		...
		a.senderId = senderId;
		// if(limit == 0)
		a.message = "Req Get " + String.valueOf(a.reqNo) + " " + fileName + " "
				+ priority;
		// else
		// a.message = "Req Aget " + String.valueOf(a.reqNo) + " " + fileName +
		// " " + priority + " " + limit;
		Write(a);
		a = Read(a);
		return a;
	...
	static DataObject Put(DataObject a, String fileName, int priority, int limit) {
		...
		a.senderId = senderId;
		// if(limit == 0)
		a.message = "Req Put " + String.valueOf(a.reqNo) + " " + fileName + " "
				+ priority;
		// else
		// a.message = "Req Aput " + String.valueOf(a.reqNo) + " " + fileName +
		// " " + priority + " " + limit;
		Write(a);
		a = Read(a);
		return a;
	...
	static boolean Pull(DataObject a, String fileName, int limit,
			ObjectOutputStream fsos, ObjectInputStream fsis) {
		// reqNo++;
		a.reqNo = reqNo;
		...
		try {
			...
			while (notLast) {
				currentChunk++;
				a.senderId = senderId;
				a.message = "Req Pull " + String.valueOf(a.reqNo) + " "
						+ fileName + " " + (offset) + " " + chunkSize * KB2B;
				Write(fsos, a);
				a = Read(fsis, a);
				if (a != null) {
					if (a.success) {
	...
	static DataObject Push(DataObject a, String fileName, int limit) {
		// reqNo++;
		a.reqNo = reqNo;
		int offset = 0;
		boolean notLast = true;
		...
		try {
			...
			while (notLast) {
				currentChunk++;
				a.senderId = senderId;
				a.message = "Req Push " + String.valueOf(a.reqNo) + " "
						+ fileName;
	...
	static DataObject Read(ObjectInputStream is, DataObject input) {
		try {
			...
			System.out.println("Server Crash Read1");
			return null;
		} catch (Exception E) {
			System.out.println("Server Crash Read2");
		}
		return input;
	}
};